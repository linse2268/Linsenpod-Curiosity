...
void setup() {
  ...
  server.on("/api/special", []() {
  String html_6 = "<head><title>counter</title><style>"
                  "body{margin: 0px;background-color: black;}"
                  "#countlbl{margin-top: 170px;display: block;text-align: center; font-size: 10em;font-family: helvetica;color: gray;background: transparent;    padding: 10px;}"
                  "#buttonContainer{"
                  "text-align: center;"
                  "}"
                  ".buttons{"
                  "padding: 10px 20px;font-size: 2em;color: gray;background-color: black;border: 2px solid gray;border-radius: 10px;margin: 10px;cursor: pointer;transition: box-shadow 0.25s;"
                  "}"
                  ".buttons:hover{box-shadow: 0px 0px 20px gray;}</style>"
                  "</head>"
                  "<body>"
                  "<label id='countlbl'>0</label>"
                  "<br>"
                  ...
                  "</div>"
                  "</body>";
  server.send(200, "text/html", html_6);
  });
